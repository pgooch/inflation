...
class inflation{
	...
	/*
		This is the historic CPI information as pulled from http://www.bls.gov/cpi/cpid1406.pdf Table 24. The data is stored by year, each one containing 
		another array with the average followed by the monthly value. With this information calculating the adjusted amount is easy. The Official Bureau 
		of Labor Statistics websites calculator uses year averages for every year except the current, where it uses the latest month. The last year will 
		not be complete until the following year, so in order to making finding the absolute latest data as easy as possible simply leave the remaining 
		months omitted. In order to keep month-to-month calculations simple set the year average as null until it can be updated, this keeps each months 
		data in order. This table and repository will probably not be updated for each months data, but if you do please feel free to make a pull request 
		with the updated information and I'll merge them. 
	*/
	...
	/*
		This will use the big-ol multi-dimensional array above to determine the inflation or deflation about of a given value between two years.

		$amount - The amount you want to determine inflation/deflation for.
		$in_year - The year of the amount.
		$like_year - The year you want to determine inflation/deflation for. If "today" is passed it will calculate with the most current information
	*/
	function adjust($amount,$in_year,$like_year='today'){
	...
	/*
		This will take a year and attempt to determine what CPI data to grab down to the month, if 'today' is passed it simply grabs the last possible month.

		$year - the year you looking for.
	*/
	function get_cpi_avg($year){
		...
		// If it's not then I guess we have to do some work.
			...
			// Now were going to check if the month is valid, if not then were going to use the last month we have that year (this should only happen if they pick a month in the future of the current year).
			if(!isset($this->cpi_data[$year][$month])){
				$month = count($this->cpi_data[$year]);
			}
	...
	/*
		This function will manually pull the inflation value from the BLS.gov CPI calculator. Because it uses the calculator it cannot do month, level 
		conversions and only takes years. Since this one does not rely on the $cpi_data it can be called statically.
